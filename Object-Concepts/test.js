...
// accessing each objects (cities)

const cities = db.map((city) => {
  //   console.log(city);

  const cityKeyVal = Object.entries(city);
  //   console.log(cityKeyVal);

  for (const [key, value] of cityKeyVal) {
    // takes care of id, name, backgroundUrl
    if (typeof value !== "object") console.log(`${key} : ${value}`);

    // takes care of weather object's keys & values
    if (key === "weather") {
    ...
    // takes care of categories object's keys & values
    if (key === "categories") {
      for (const [i, j] of Object.entries(value)) {
        const ans = j.map((x) => {
          for (let l in x) {
